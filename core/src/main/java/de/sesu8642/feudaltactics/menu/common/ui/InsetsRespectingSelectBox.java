...
/**
 * Copy of {@link SelectBox} that considers device insets when determining whether to display the option list above or below. It does not contain the {@link SelectBox.SelectBoxStyle} because the original can be used here. This is not build using inheritance because that was a PITA when I attempted it.
 * Original JavaDoc:
 ...
 * @author Nathan Sweet
 */
public class InsetsRespectingSelectBox<T> extends Widget implements Disableable {
	...
	public InsetsRespectingSelectBox(Skin skin, PlatformInsetsProvider platformInsetsProvider) {
		this(skin.get(SelectBox.SelectBoxStyle.class), platformInsetsProvider);
	}

	public InsetsRespectingSelectBox(Skin skin, String styleName, PlatformInsetsProvider platformInsetsProvider) {
		this(skin.get(styleName, SelectBox.SelectBoxStyle.class), platformInsetsProvider);
	}

	public InsetsRespectingSelectBox(SelectBox.SelectBoxStyle style, PlatformInsetsProvider platformInsetsProvider) {
	...
	/** Returns the first selected item, or null. For multiple selections use {@link InsetsRespectingSelectBox#getSelection()}. */
	...
	static public class SelectBoxScrollPane<T> extends ScrollPane {
		final InsetsRespectingSelectBox<T> selectBox;
		int maxListCount;
		...
		public SelectBoxScrollPane (final InsetsRespectingSelectBox<T> selectBox) {
		 ...
		 * {@link List#toString(Object)} to {@link InsetsRespectingSelectBox#toString(Object)}. */
		...
		public void show (Stage stage) {
            ...
            Insets insets = selectBox.platformInsetsProvider.getInsets(Gdx.app);
            float heightBelow = stagePosition.y - insets.getBottomInset();
			float heightAbove = stage.getHeight() - heightBelow - selectBox.getHeight() - insets.getTopInset() - insets.getBottomInset();
			boolean below = true;
		...
		public InsetsRespectingSelectBox<T> getSelectBox () {
			return selectBox;
		}
	}
